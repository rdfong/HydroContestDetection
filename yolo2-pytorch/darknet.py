...
class Darknet19(nn.Module):
    def __init__(self):
        ...
        #self.hFC3 = net_utils.FC(32, 9*6*26*26);
    ...
    def forward(self, im_data, horizon, gt_boxes=None, gt_classes=None, dontcare=None):
        ...
        #Feed forward part of horizon line network
        #h0 = self.hFC1(horizon) #BATCH SIZE, 32
        #h1 = self.hFC2(h0) #BATCHSIZE, 32
        #h2 = self.hFC3(h1)
        #h2 = h2.view(bsize, 26*26, cfg.num_anchors, 6)
        #temp = torch.add(conv5_reshaped[:,:,:,4:10],h2)
        #conv5_reshaped = torch.cat((conv5_reshaped[:,:,:,0:4], temp), 3)
