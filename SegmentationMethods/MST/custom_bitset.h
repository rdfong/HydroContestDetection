...
template<size_t _Nb>
class custom_bitset : private _Base_bitset<__BITSET_WORDS(_Nb)>
{
private:
...
public:
  ...
  public:
    reference( custom_bitset& __b, size_t __pos ) {
  ...
  custom_bitset() {}
  custom_bitset(unsigned long __val) : _Base_bitset<__BITSET_WORDS(_Nb)>(__val)
    { _M_do_sanitize(); }

#ifdef __STL_MEMBER_TEMPLATES
  template<class _CharT, class _Traits, class _Alloc>
  explicit custom_bitset(const basic_string<_CharT, _Traits, _Alloc>& __s,
                  size_t __pos = 0)
    : _Base()
  {
    if (__pos > __s.size())
      __STL_THROW(out_of_range("custom_bitset"));
    _M_copy_from_string(__s, __pos,
                        basic_string<_CharT, _Traits, _Alloc>::npos);
  }
  template<class _CharT, class _Traits, class _Alloc>
  custom_bitset(const basic_string<_CharT, _Traits, _Alloc>& __s,
         size_t __pos,
         size_t __n)
    : _Base()
  {
  ...
  explicit custom_bitset(const basic_string<char>& __s,
                  size_t __pos = 0,
                  size_t __n = basic_string<char>::npos)
    : _Base()
  {
    if (__pos > __s.size())
      throw(out_of_range("custom_bitset"));
    _M_copy_from_string(__s, __pos, __n);
  }
#endif /* __STL_MEMBER_TEMPLATES */

  // 23.3.5.2 bitset operations:
  custom_bitset<_Nb>& operator&=(const custom_bitset<_Nb>& __rhs) {
  ...
  custom_bitset<_Nb>& operator<<=(size_t __pos) {
  ...
  custom_bitset<_Nb>& _Unchecked_set(size_t __pos) {
    this->_M_getword(__pos) |= _Base::_S_maskbit(__pos);
    return *this;
  }

  custom_bitset<_Nb>& _Unchecked_set(size_t __pos, int __val) {
    if (__val)
      this->_M_getword(__pos) |= _Base::_S_maskbit(__pos);
    else
      this->_M_getword(__pos) &= ~_Base::_S_maskbit(__pos);
  ...
  custom_bitset<_Nb>& _Unchecked_reset(size_t __pos) {
    this->_M_getword(__pos) &= ~_Base::_S_maskbit(__pos);
    return *this;
  }

  custom_bitset<_Nb>& _Unchecked_flip(size_t __pos) {
    this->_M_getword(__pos) ^= _Base::_S_maskbit(__pos);
    return *this;
  }
  ...
  custom_bitset<_Nb>& set() {
    this->_M_do_set();
    this->_M_do_sanitize();
    return *this;
  }

  custom_bitset<_Nb>& set(size_t __pos) {
    if (__pos >= _Nb)
      throw(out_of_range("custom_bitset"));
  ...
  custom_bitset<_Nb>& set(size_t __pos, int __val) {
    if (__pos >= _Nb)
      throw(out_of_range("custom_bitset"));
  ...
  custom_bitset<_Nb>& reset() {
    this->_M_do_reset();
    return *this;
  }

  custom_bitset<_Nb>& reset(size_t __pos) {
    if (__pos >= _Nb)
      throw(out_of_range("custom_bitset"));
  ...
  custom_bitset<_Nb>& flip() {
    this->_M_do_flip();
    this->_M_do_sanitize();
    return *this;
  }

  custom_bitset<_Nb>& flip(size_t __pos) {
    if (__pos >= _Nb)
      throw(out_of_range("custom_bitset"));
  ...
  custom_bitset<_Nb> operator~() const {
    return custom_bitset<_Nb>(*this).flip();
  }
  ...
  bool operator==(const custom_bitset<_Nb>& __rhs) const {
  ...
  custom_bitset<_Nb> operator<<(size_t __pos) const
    { return custom_bitset<_Nb>(*this) <<= __pos; }
  custom_bitset<_Nb> operator>>(size_t __pos) const
    { return custom_bitset<_Nb>(*this) >>= __pos; }
                        ...
                        size_t __n)
{
  ...
  for (size_t __i = 0; __i < __nbits; ++__i) {
    ...
    case '1':
      ...
      break;
    default:
      throw(invalid_argument("custom_bitset"));
    }
  }
}
...
template <size_t _Nb>
void custom_bitset<_Nb>::_M_copy_from_string(const basic_string<char>& __s,
...
template <size_t _Nb>
void custom_bitset<_Nb>::_M_copy_to_string(basic_string<char>& __s) const
{
  __s.assign(_Nb, '0');
...
//
// 23.3.5.3 custom_bitset operations:
//

template <size_t _Nb>
inline custom_bitset<_Nb> operator&(const custom_bitset<_Nb>& __x, const custom_bitset<_Nb>& __y) {
  custom_bitset<_Nb> __result(__x);
  __result &= __y;
  return __result;
}
...
operator>>(basic_istream<_CharT, _Traits>& __is, custom_bitset<_Nb>& __x)
{
...
operator<<(basic_ostream<_CharT, _Traits>& __os, const custom_bitset<_Nb>& __x)
{
  basic_string<_CharT, _Traits> __tmp;
...
template <size_t _Nb>
istream& operator>>(istream& __is, custom_bitset<_Nb>& __x) {
  string __tmp;
  __tmp.reserve(_Nb);
...
template <size_t _Nb>
ostream& operator<<(ostream& __os, const custom_bitset<_Nb>& __x) {
  string __tmp;
  __x._M_copy_to_string(__tmp);
  return __os << __tmp;
}
