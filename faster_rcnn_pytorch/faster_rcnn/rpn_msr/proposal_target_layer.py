...
def _sample_rois(all_rois, gt_boxes, gt_ishard, dontcare_areas, fg_rois_per_image, rois_per_image, num_classes):
	...
	rois = all_rois[keep_inds]
       ...
       labels.fill(0)
       bg_inds = np.asarray(range(len(all_rois)))
       bg_rois_per_this_image = min(rois_per_image, bg_inds.size)
       bg_inds = npr.choice(bg_inds, size=bg_rois_per_this_image, replace=False)
       labels = labels[bg_inds]
       rois = all_rois[bg_inds]


    bbox_target_data = np.zeros((len(rois),5))
    if len(gt_boxes) > 0:
